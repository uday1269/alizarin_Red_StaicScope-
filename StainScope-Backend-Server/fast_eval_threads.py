...
def detect_nodules_opt(image, binary_mask, cfg=NoduleDetectionConfig(), threshold_rel_int=0.45):
    ...
    # Local intensity minima (dark nodule cores) with relative threshold 0.45
...
def process_one(gt_path):
    ...
    if bgr is None or gt_mask is None: return None
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        raw_results = list(executor.map(process_one, gt_files))
    ...
    c6_dt = np.mean([r["dt"] for r in results if r["cat"] == "c6"])
    c7_dt = np.mean([r["dt"] for r in results if r["cat"] == "c7"])
    c8_dt = np.mean([r["dt"] for r in results if r["cat"] == "c8"])
    
    print("\n==================================================")
    print("OPTIMIZED BENCHMARK RESULTS (72 Ground Truth Images)")
    ...
    print(f"C6 Mean Image Time:   {c6_dt:.3f}s")
    ...
    summary = {
        "summary": {
            ...
            "c6_mean_time": round(float(c6_dt), 3),
    ...
    with open("opt_72_benchmark_summary.json", "w") as f:
